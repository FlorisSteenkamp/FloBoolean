...
function getIntersection(
    ...
    if (ris2 === undefined) {
        ...
        for (const xPair of xPairs) {
            ...
            const ri1: _X_ = { x: { ri: xPair.ri1, kind: 5, p }, curve: curveA, next: undefined!, prev: undefined!, container: undefined!, order: 0 }; // exact overlap endpoint
            const ri2: _X_ = { x: { ri: xPair.ri2, kind: 5, p }, curve: curveB, next: undefined!, prev: undefined!, container: undefined!, order: 0 }; // exact overlap endpoint
    ...
    for (const xPair of xPairs) {
        const x1: _X_ = { x: xPair[0], curve: curveA, next: undefined!, prev: undefined!, container: undefined!, order: 0 };
        const x2: _X_ = { x: xPair[1], curve: curveB, next: undefined!, prev: undefined!, container: undefined!, order: 0 };
        xs.push([x1, x2]);
    }
