 ...
 */
function inResult(
        booleanOp: 'AND' | 'OR' | 'XOR' | undefined,
        windingNum: number): boolean {

    if (booleanOp === 'AND') { return abs(windingNum) >= 2; }
    if (booleanOp === 'XOR') { return (abs(windingNum) % 2) === 1; }
    // 'OR' and the default
    return windingNum !== 0;
}
...
function getLoopsFromTree(
    ...
    return (root: Out): { out: Out; depth: number }[] => {
        ...
        while (stack.length) {
            ...
            const isSelected =
                inResult(booleanOp, tree.windingNum) !==
