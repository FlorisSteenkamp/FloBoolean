...
function drawContainer(g, container, classes, delay = 0, drawBigBox = false) {
    ...
    const $texts = [];
    const placed = []; // label anchor points already used
    ...
    for (let i = 0; i < inOuts.length; i++) {
        ...
        if (inOut.idx !== undefined) {
            ...
            const overlaps = placed.filter(q => Math.hypot(q[0] - p[0], q[1] - p[1]) < scale / 4).length;
            placed.push(p);
 ...
 */
function getInOutBoxCrossing(inOut, box) {
